...
export default function RuberraShell() {
  ...
  // Holds the latest context from whichever SideRail is active
  const chamberContextRef = useRef<ChamberContext>({});
  const handleContextChange = useCallback((ctx: ChamberContext) => {
    chamberContextRef.current = ctx;
  }, []);

  const handleSubmit = useCallback(
    async (text: string) => {
      const tab = activeTab;
      const context = chamberContextRef.current;
      ...
      setMessages((prev) => [
        ...prev,
        { id: assistantId, role: "assistant", content: "", tab, timestamp: Date.now(), streaming: true },
      ]);
      ...
      try {
        ...
        const res = await fetch("/api/chat", {
          ...
          body: JSON.stringify({ tab, messages: history, context }),
        });
