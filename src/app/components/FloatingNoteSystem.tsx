...
function NoteCard({ note, onChange, onRemove }: { note: FloatingNote; onChange: (u: Partial<FloatingNote>) => void; onRemove: () => void; }) {
  ...
  return (
    <div
      ref={cardRef}
      style={{ position: 'fixed', zIndex: 50, width: '240px', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 24px rgba(0,0,0,0.12)', left: note.x, top: note.y, background: 'var(--r-elevated)', border: '1px solid var(--r-border)', borderTop: `2px solid ${tabAccent[note.tab]}`, borderRadius: '3px', cursor: dragging ? 'grabbing' : 'grab', userSelect: 'none' }}
    ...
    >
      ...
      <textarea
        ...
        placeholder="Note…"
        rows={4}
        style={{ width: '100%', background: 'transparent', border: 'none', outline: 'none', resize: 'none', fontSize: '11px', lineHeight: '1.6', padding: '8px 10px', color: 'var(--r-text)', cursor: 'text', userSelect: 'text', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }}
      />
