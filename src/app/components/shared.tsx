/**
 * RUBERRA Shared Components — atomic primitives
 */
...
export function RIconButton({ title, onClick, children, active = false, size = 30, style, className }: {
  ...
  return (
    <button title={title} onClick={onClick}
      style={{ width: `${size}px`, height: `${size}px`, borderRadius: R.r.lg, border: `1px solid ${active ? R.strong : R.hairline}`, background: active ? R.selected : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', outline: 'none', transition: 'background 0.12s ease', flexShrink: 0, ...style }}
...
export function RDivider({ vertical = false, style }: { vertical?: boolean; style?: React.CSSProperties }) {
  return (
    <div style={{ background: R.hairline, ...(vertical ? { width: '1px', alignSelf: 'stretch' } : { height: '1px', width: '100%' }), flexShrink: 0, ...style }} />
  );
}

export function RChamberTag({ label, color }: { label: string; color: string }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '2px 8px', borderRadius: R.r.sm, border: `1px solid ${R.hairline}`, background: R.shell, ...R.t.label, color, fontFamily: "'Inter', sans-serif", textTransform: 'uppercase' as const }}>
...
export function RSegmentDots({ total, filled, color = R.ink }: { total: number; filled: number; color?: string }) {
  return (
    <div style={{ display: 'flex', gap: '3px' }}>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{ width: '14px', height: '2px', borderRadius: '1px', background: i < filled ? color : R.hairline }} />
      ))}
    </div>
  );
}
